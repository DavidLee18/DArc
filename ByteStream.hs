...
class BufferData a where
  ...
  writeList buffer =  mapM_ (write buffer)
...
readInteger  buf  =  read buf Data.Functor.<&> fromInteger
...
unpackCharUtf8 buf pos ref_pos = do
      ...
      ch0 <- fromIntegral fmap peekElemOff addr pos
      case () of
         _ | ch0 <= 0x7F -> do
                ref_pos =: pos+1
           ...
           | ch0 <= 0xDF -> do
                ref_pos =: pos+2
                ch1 <- fromIntegral fmap peekElemOff addr (pos+1)
                return $! unsafeChr (((ch0 - 0xC0) `shiftL` 6) +
                                       (ch1 - 0x80))
           | ch0 <= 0xEF -> do
                ref_pos =: pos+3
                ch1 <- fromIntegral fmap peekElemOff addr (pos+1)
                ch2 <- fromIntegral fmap peekElemOff addr (pos+2)
           ...
           | otherwise -> do
                ...
                ch1 <- fromIntegral fmap peekElemOff addr (pos+1)
                ch2 <- fromIntegral fmap peekElemOff addr (pos+2)
                ch3 <- fromIntegral fmap peekElemOff addr (pos+3)
...
unpackCStringUtf8 buf pos ref_pos = do
  ...
  where
    ...
    unpack nh = do
      ch0 <- fromIntegral fmap peekElemOff addr nh
      case () of
         _ | ch0 == 0 -> do
                ref_pos =: nh + 1
                return []
           ...
           | ch0 <= 0xDF -> do
                ch1 <- fromIntegral fmap peekElemOff addr (nh+1)
                chs <- unpack (nh + 2)
                return $! (unsafeChr (((ch0 - 0xC0) `shiftL` 6) +
                                       (ch1 - 0x80)) : chs)
           | ch0 <= 0xEF -> do
                ch1 <- fromIntegral fmap peekElemOff addr (nh+1)
                ch2 <- fromIntegral fmap peekElemOff addr (nh+2)
                chs <- unpack (nh + 3)
                return $! (unsafeChr (((ch0 - 0xE0) `shiftL` 12) +
                                      ((ch1 - 0x80) `shiftL` 6) +
                                       (ch2 - 0x80)) : chs)
           | otherwise -> do
                ch1 <- fromIntegral fmap peekElemOff addr (nh+1)
                ch2 <- fromIntegral fmap peekElemOff addr (nh+2)
                ch3 <- fromIntegral fmap peekElemOff addr (nh+3)
                chs <- unpack (nh + 4)
                return $! (unsafeChr (((ch0 - 0xF0) `shiftL` 18) +
                                      ((ch1 - 0x80) `shiftL` 12) +
                                      ((ch2 - 0x80) `shiftL` 6) +
