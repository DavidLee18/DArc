...
pub fn parse(header: &[u8]) -> Result<BlockHeader, i32> {
    ...
    // mode == 0 is "stored"; it carries no stage selection to validate.
    if h.is_stored() {
        return Ok(h);
    }

    // The C rebuilds `mode` from the unpacked fields and requires it to match,
    // which rejects any combination the encoder could not have produced --
    // including reserved sorter and coder values.
...
mod tests {
    ...
    fn mode_unpacks_into_the_four_stage_fields() {
        ...
        let h = parse(&make(1000, 900, mode, 1)).unwrap();
