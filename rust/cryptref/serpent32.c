...
#define ROL(x,y) ((ulong32)(((ulong32)(x)<<(ulong32)((y)&31))|(((ulong32)(x)&0xFFFFFFFFUL)>>(ulong32)(32-((y)&31)))))
#define LOAD32L(x,y) do { memcpy(&(x),(y),4); } while(0)
#define STORE32L(x,y) do { ulong32 __t=(x); memcpy((y),&__t,4); } while(0)
static ulong32 l_key[140];
...
#define rot(a,b,c,d) a=ROL(a,13); c=ROL(c,3); d^=c^(a<<3); b^=a^c; d=ROL(d,7); b=ROL(b,1); a^=b^d; c^=d^(b<<7); a=ROL(a,5); c=ROL(c,22)
...
#define ib7(a,b,c,d,e,f,g,h)    \
    ...
    t16 = e ^ t14;  \
    g = t15 ^ t16
static void setup(const unsigned char* key, int keybytes) {
    ulong32 a,b,c,d,e,f,g,h;
    ulong32 t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13,t14,t15,t16;
    int key_len = keybytes*8;
    ulong32 i=0, lk=(key_len+31)/32;
    memset(l_key,0,sizeof l_key);
    for (i=0;i<lk;i++) memcpy(&l_key[i], key+4*i, 4);
    if (key_len < 256) { i=key_len/32; lk=1u<<(key_len%32); l_key[i]=(l_key[i]&(lk-1))|lk; }
    for (i=0;i<132;++i){
        lk = l_key[i]^l_key[i+3]^l_key[i+5]^l_key[i+7]^0x9e3779b9u^i;
...
static void encrypt(const unsigned char* pt, unsigned char* ct) {
    ulong32 a,b,c,d,e,f,g,h;
    ulong32 t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13,t14,t15,t16;
...
static void one(const char* name, const unsigned char* key, int kl, const unsigned char* pt) {
    ...
    for (int i=0;i<16;i++) printf("%02x",ct[i]); printf("\n");
}
int main(){
    printf("# sizeof(ulong32)=%zu\n", sizeof(ulong32));
