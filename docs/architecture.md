...
`Environment.cpp` (62KB, 1,917 lines — the largest C++ file left) provided OS-level services to the Haskell side: file/console/memory primitives that differ across Windows and Unix. It belonged with the Haskell layer rather than the codecs, which is why the archiver no longer reaches it at all — the port uses `std` for the same work. Nothing compiles against it now either: `Unarc/` was its last caller. It is a deletion candidate, not a dependency.
