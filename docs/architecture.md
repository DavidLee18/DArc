...
- **`Unarc/`** — the standalone extractor and the SFX modules embedded into self-extracting archives. **The Linux side is ported.** `make -C Unarc linux` compiles no C++ at all: it builds `rust/darc-unarc` with cargo and installs that one binary as both `unarc` and `arc.linux.sfx`.

  **The Windows side is not ported**, and is what keeps `unarc.cpp`, `ArcStructure.h` and `CUI.h` in the tree — `make windows` compiles `unarc.cpp` four more times for the console SFX, the GUI SFX and the installer stub, and `FarPlugin.cpp` for `FreeArc.fmt`, the FAR Manager plugin. No CI job builds any of them.

  The C++ extractor is **not deleted; it is demoted to an oracle.** `make -C Unarc oracle` builds it as `unarc-c` / `arc-c.linux.sfx` — never shipped, named so that nothing can pick it up believing it is the product — which is the role `Tests/arc-ghc` plays for the Haskell reference. It is what `rust/difftest/unarc-check.sh` differs against, and the only C binary left on Linux for the `asan-x86_64` job to sanitize. That oracle build is a **`DARC_RUST` build** and links `rust/target/release/libdarc_codecs.a` (built with the `dropin` feature, and placed *after* the objects — GNU ld resolves an archive only against undefineds it has already seen); most codecs it needs no longer have a C decoder, so restoring the C to feed it was never an option.

  Why it was ported: `ArcStructure.h` was a **second implementation of the archive reader**, and it disagreed with the writer. It read the per-file time field as **4 bytes** where `ByteStream` writes `CTime` as a fixed 64-bit value, so the directory flags and the CRCs stored after it all came out of the wrong offset — directories were recreated as zero-byte *files* and every extracted file failed its CRC, while a listing looked perfect throughout because names and sizes are stored *before* the time field. `rust/darc-unarc` owns no format knowledge: it reads argv and calls `darc-arc`.

  What *is* true and checkable: its codecs are already Rust — the makefile builds `-DDARC_RUST` and links `libdarc_codecs.a`, and the `unarc-sfx` job greps the stripped binary for `darc-codecs/src/` panic markers to prove it. What remains C++ is ~3,700 lines of structure parser and CLI shell.

  The argument for keeping it: **it is a second, independent implementation of the archive READER.** It shares the codecs with `arc` but *not* the structure parser — that is Rust (`darc-arc/src/directory.rs`) in the archiver and C++ (`Unarc/ArcStructure.h`) here — so the two cross-check each other on a format with no written spec. The argument against: a second implementation nothing exercised is how it drifted out of sync in the first place, and the deleted `RUST_PORT_PROGRESS.md` spoke of the C `DisFilter` staying "for `Unarc/` *until the directory is deleted*", so removal was at least contemplated. Nothing else in CI exercised the C++ reader, which is why it was able to drift and stay drifted. The `unarc-sfx` job now builds it and round-trips through it, including running a real self-extracting archive; a build-only job would not have caught the bug that made it useless.
