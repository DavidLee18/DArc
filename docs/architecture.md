...
- **`ArhiveStructure.hs`** — the physical layout. An archive is a sequence of blocks: `BlockType` (`:313`) is `DESCR_BLOCK`, `HEADER_BLOCK`, `DATA_BLOCK`, `DIR_BLOCK`, `FOOTER_BLOCK`, `RECOVERY_BLOCK`. Every non-data block is followed by a *descriptor* that allows locating and reading it even when the archive is partially corrupt — this is what makes `arc r` possible. See `aSIGNATURE` (`:37`), `ArchiveBlock` (`:273`), `FooterBlock` (`:214`).
  ...
  The footer's body is the **one place the format extends without moving a byte**: it ends in optional trailing fields, and a reader that runs out of bytes defaults the rest rather than failing. That is already how an archive written before the recovery field, or before the UTF-8 comment, still reads today. `autorun` (#154) is the port's own addition on the same principle — written only when non-empty, so an archive that does not use it is byte-identical to one from before the field existed. Anything else added here must follow that rule: append, never insert, and never write an empty value.
...
> **`Compression/` is not in the working tree.** Every path under it in this document refers to one of the two pinned trees — `DARC_C_REF_SHA` for the original C, `DARC_WRAPPER_REF_SHA` for the forwarders — which `rust/difftest` extracts on demand. Nothing here is a file you can open in a checkout, and nothing here is a file you can delete.

Two directories look dead and are **not**, which matters when reading the pinned tree: `Compression/LZMA/Common` and `Windows` (a 7-Zip portability layer that `Compression/MultiThreading.h` and `CompressionLibrary.cpp` include), and `Compression/Tornado/Tornado.cpp` (`C_Tornado.cpp` does `#include "Tornado.cpp"`).
...
`Environment.cpp` — **deleted**, along with `Environment.h`, `Win32Files.c` and `Win32Files.h`. It provided OS-level services to the Haskell side: file/console/memory primitives that differ across Windows and Unix. It belonged with the Haskell layer rather than the codecs, and the port uses `std` for the same work; `Unarc/` was its last compiler, so it went when that did. Read it at `c267621` — `systemRandomData` in particular, which two comments still cite as the entropy source Fortuna was seeded from.
...
- **`Unarc/` — deleted.** It held the standalone extractor, the SFX modules, the Windows GUI, the installer stub and the FAR Manager plugin: 29 files, 8,008 lines. `rust/darc-unarc` replaced all of it with one cargo binary that is both `unarc` and the SFX module, deciding at runtime — through `current_exe()` — whether an archive is appended to itself. There is no compile-time SFX flag and no tiered stub.

  **Why, and it is the shape the next such decision should take:** `unarc.cpp` + `ArcStructure.h` + `CUI.h` were a *second, independent implementation of the archive reader*. That was once argued as a virtue — two readers cross-checking a format with no written spec — but the format is not the part that drifted. `ArcStructure.h` read the per-file time field as **4 bytes** where `ByteStream` writes `CTime` as a fixed 64-bit value, so the directory flags and CRCs stored after it came out of the wrong offset: directories were recreated as zero-byte *files* and every extracted file failed its CRC, while a listing looked perfect throughout because names and sizes are stored *before* the time field. A second implementation that nothing exercised is how it drifted and stayed drifted. `rust/darc-unarc` owns no format knowledge at all — it reads argv and calls `darc-arc`, so there is nothing left to keep in sync.

  The C++ was kept briefly as a differential oracle (`make -C Unarc oracle`) and then deleted too: git history holds it, `rust/difftest/c-reference.sh` already fetches C from a pinned SHA rather than the tree, and the migration it gated is over. The `asan-x86_64` CI job went with it — it sanitized that binary, and `-fsanitize=address` has nothing to instrument in a Rust one.

  What went with it that was never rebuilt by CI: `make windows` produced the console SFX, the GUI SFX (`gui/`), the installer stub and `FreeArc.fmt`. No workflow ever invoked it. The Windows SFX module is now the cross-compiled `unarc.exe`, built by the `windows-cross` job.

  **The installer stub came back, as an option rather than a build** (#154). `darc a -sfx<module> --autorun'CMD'` records `CMD` in the footer; the stub extracts itself into a fresh `%TEMP%/darc-sfx-<pid>-<n>`, asks, runs `CMD` from there, propagates its exit code and wipes the directory. `rust/darc-unarc/src/autorun.rs`, gated by `rust/difftest/sfx-autorun-check.sh`.

  Four things differ from `-DFREEARC_INSTALLER` deliberately, and the reasons generalise. The command is **in the archive**, because a second cargo output collides on `target/release/unarc` — that is #149, not a hypothetical. It **asks first**, because silently running a payload on the most likely user action is the shape of malware rather than of an installer; `-y` says yes in advance and EOF says no, which matters most exactly where a prompt is not seen. The command must **resolve inside the extracted tree**, checked when the archive is written *and* again before the spawn, since the archive in hand need not have been written by this build. And the child's **exit code is propagated**, where the C discarded it and reported success for a failed install.

  Kept from the original because it was right: `create_dir` in a unique-name loop, never `create_dir_all`. Failing when the directory already exists is what stops the payload being extracted into a tree someone else planted. The mode is `0700` set *at creation* through `DirBuilder`, not by a `set_permissions` afterwards — between those two calls the directory is world-readable, and the files land in it during exactly that window.
...
- **`Installer/` — deleted.** It held FreeArc's NSIS installer scripts, the `release*.cmd` packaging batch files, 15 `arc.languages/*.txt` translations, and `bin/` with the original `arc.ini`, `arc.groups` and `freearc.history`: 46 files, 12,363 lines. None of it was reachable — the port has no localisation (nothing reads `arc.languages`), the packaging scripts named `FreeArc.fmt` and `*.sfx` targets that no longer exist, and `bin/arc.groups` is byte-identical to `Tests/darc.groups`, which is the copy that is actually read. The `arc.ini` that two SREP comments cited for `default = m3f` is at `c267621`.
